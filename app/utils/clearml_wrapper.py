...
def init_task(task_name: str, task_type: str = "training") -> Task:
    """Initialise a ClearML task for experiment tracking."""
    task = Task.init(
...
def list_published_models() -> List[Dict[str, Any]]:
    """Return metadata for all completed/published ClearML tasks that have a model."""
        ...
        models_data.append(
            {
                ...
                "feature_columns": (
                    metadata.get("feature_columns", "").split(",")
...
def load_model_from_clearml(task_id: str):
    """Download and deserialise a model artifact by ClearML task ID."""
...
def get_task_metadata(task_id: str) -> Dict[str, Any]:
    """Retrieve training metadata stored inside a ClearML task."""
    task = Task.get_task(task_id=task_id)
    if not task:
        raise ValueError(f"Task {task_id} not found")
